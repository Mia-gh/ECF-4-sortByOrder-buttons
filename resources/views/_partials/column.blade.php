<!-- board header -->
<div
    class="sticky top-16 flex flex-row justify-between items-center py-3 px-2 backdrop-blur-md bg-white/30 border-t-4 border-{{ $color }}-400">
    <div class="flex items-center ">
        <h2
            class="inline-flex items-center rounded-md bg-{{ $color }}-50 px-2 py-1 text-xs font-medium text-{{ $color }}-700 ring-1 ring-inset ring-{{ $color }}-600/20">
    ...
    @foreach (${'tasks_' . $status} as $task)
        <div x-data="{ openModale: false }" @click="openModale = true"
            ...
            <p class="text-gray-400 text-sm mt-3 text-right">
                {{ Carbon\Carbon::parse($task->end_date)->diffForHumans() }}</p>
...
<!-- adding card -->
<div @click="openModale = true; status = '{{ Str::replace('_', ' ', $status) }}'"
    class="sticky top-32 flex flex-row items-center text-gray-900 px-2 backdrop-blur-md bg-white/30 cursor-pointer hover:bg-white/60 transition-all duration-200">
